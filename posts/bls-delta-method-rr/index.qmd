...
The two intervals diverge most when the baseline rate $p_0$ is low. A low $p_0$ means few positive responses in the control group. Because $\hat{p}_0$ sits in the denominator of $\widehat{RR}$, a small downward fluctuation in $\hat{p}_0$ inflates $\widehat{RR}$ far more than an equal upward fluctuation shrinks it. The resulting sampling distribution is right-skewed. The naive CI assumes symmetry; the log-transform CI does not. The log-transform CI is still an asymptotic method. It requires $x_1$ and $x_0$ to be at least moderate — roughly 5 or more. Below that, the stated confidence level is no longer reliable.
