...
The two intervals diverge most when the baseline rate $p_0$ is low. Few positive responses in the control group make $\hat{p}_0$ unstable, and the sampling distribution of $\widehat{RR}$ is no longer symmetric. The naive CI cannot reflect this; the log-transform CI can, because it is asymmetric by construction.

However, the log-transform CI is still an asymptotic method. It requires $x_1$ and $x_0$ to be at least moderate — roughly 5 or more. Below that, the stated confidence level is no longer reliable.
