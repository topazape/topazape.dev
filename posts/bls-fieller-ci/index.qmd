...
The delta method is the only method that produces a closed-form variance formula. The [first post](/posts/bls-delta-method-rr/) derived $\text{Var}(\log(\widehat{RR})) = \frac{1}{x_1} - \frac{1}{n_1} + \frac{1}{x_0} - \frac{1}{n_0}$, and the [second post](/posts/bls-bootstrap-validation/) used it to solve for $n_{\min}$ by inverting the power equation. Meta-analysis weights each study by $w_k = 1/\text{Var}(\log(\widehat{RR}_k))$, again requiring the same formula. The bootstrap can estimate a variance numerically ($\text{Var}(\log(\widehat{RR}^*))$ across replicates), but this estimate cannot be inverted to solve for $n$ and is not available in SQL. Fieller returns a CI but no variance at all.

BLS platforms compute CIs for hundreds of studies in batch SQL pipelines. The delta method and Fieller both reduce to arithmetic expressions: `SQRT`, `LN`, and `POW` in BigQuery. The bootstrap requires generating $B$ random draws per study, sorting them, and extracting quantiles. This is feasible in R or Python batch jobs but does not fit into a single SQL query.
