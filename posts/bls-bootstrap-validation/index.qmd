...
The short version: when the expected number of positive control responses $\mathbb{E}[x_0] = n \cdot p_0$ is 20 or more, the delta method CI achieves its nominal 95% coverage. Below that, it becomes conservative, producing intervals wider than the stated level implies. The second half of the post inverts the variance formula to get the minimum per-group sample size $n_{\min}$ for a target CI precision.
...
The delta method CI is reliable when $\mathbb{E}[x_0]$ is large enough. The practical question: how many respondents per group does a study need?

### Precision on the log scale

Define the CI half-width on the log scale as $\delta$:

$$\delta = z_{1-\alpha/2} \cdot SE = z_{1-\alpha/2} \sqrt{\frac{1-p_1}{n \, p_1} + \frac{1-p_0}{n \, p_0}}$$

The log-transform CI is $\log(\widehat{RR}) \pm \delta$. Exponentiating, the upper bound is $\widehat{RR} \cdot e^{\delta}$ and the lower bound is $\widehat{RR} \cdot e^{-\delta}$. Their ratio:

$$\frac{\text{CI upper}}{\text{CI lower}} = e^{2\delta}$$

This ratio is independent of $\widehat{RR}$. At $\delta = 0.20$, the upper bound is at most $e^{0.40} \approx 1.49$ times the lower bound. At $\delta = 0.10$, the ratio is $e^{0.20} \approx 1.22$.

### Derivation of $n_{\min}$

Set $n_1 = n_0 = n$ and require $\delta$ not to exceed a target:

$$z_{1-\alpha/2} \sqrt{\frac{1-p_1}{n \, p_1} + \frac{1-p_0}{n \, p_0}} \leq \delta$$

Square both sides and solve for $n$:

$$n_{\min} = \left\lceil \frac{z_{1-\alpha/2}^2}{\delta^2}\left[\frac{1-p_1}{p_1} + \frac{1-p_0}{p_0}\right] \right\rceil$$

Since $p_1 = RR \cdot p_0$, the formula takes four inputs: $p_0$, $RR$, $\delta$, and $\alpha$. This formula is valid when $n_{\min} \cdot p_0 \ge 20$, the threshold established by the coverage simulation above.

### Worked example

Planning inputs from historical study norms: $p_0 = 0.10$, $RR = 1.20$ (20% relative lift). Target precision: $\delta = 0.20$ at $\alpha = 0.05$. Then $p_1 = 0.12$ and:

$$n_{\min} = \left\lceil \frac{1.96^2}{0.20^2}\left[\frac{1-0.12}{0.12} + \frac{1-0.10}{0.10}\right] \right\rceil = \left\lceil 96.04 \times 16.333 \right\rceil = 1{,}569$$

At $\delta = 0.20$ the CI upper bound is at most $1.49\times$ the lower bound.

$p_0$ and $RR$ are planning assumptions, typically from historical norms for the relevant study category. If the actual values deviate, the realized CI precision will differ from $\delta$.
...
bls_nmin <- function(p0, rr, delta, alpha = 0.05) {
  p1 <- rr * p0
  z <- qnorm(1 - alpha / 2)
  n <- ceiling(z^2 / delta^2 * ((1 - p1) / p1 + (1 - p0) / p0))
  threshold <- 20
  n_floor <- ceiling(threshold / p0)
  max(n, n_floor)
}
```
...
```{r}
#| label: tbl-nmin
#| tbl-cap: "Minimum respondents per group ($n_{\\min}$) at $\\delta = 0.20, \\alpha = 0.05$. The CI upper bound is at most $1.49\\times$ the lower bound."
#| echo: false
#| message: false
...
nmin_grid <- crossing(
  ...
  mutate(n_min = map2_int(p0, rr, \(p0, rr) bls_nmin(p0, rr, delta = 0.20)))
...
bls_nmin(p0 = 0.10, rr = 1.20, delta = 0.20)
# [1] 1569
```